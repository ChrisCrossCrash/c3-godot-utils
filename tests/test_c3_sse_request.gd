...
class TestDrainBuffer:
	...
	func test_complete_event_clears_buffer() -> void:
		_sse._buffer = "data: hello\n\n".to_utf8_buffer()
		_sse._drain_buffer()
		assert_eq(_sse._buffer.get_string_from_utf8(), "")
	...
	func test_partial_event_not_emitted() -> void:
		_sse._buffer = "data: no-terminator".to_utf8_buffer()
		_sse._drain_buffer()
		assert_signal_not_emitted(_sse, "event_received")
	...
	func test_partial_event_preserved_in_buffer() -> void:
		_sse._buffer = "data: no-terminator".to_utf8_buffer()
		_sse._drain_buffer()
		assert_eq(_sse._buffer.get_string_from_utf8(), "data: no-terminator")

	func test_partial_tail_preserved_after_complete_event() -> void:
		_sse._buffer = "data: done\n\ndata: partial".to_utf8_buffer()
		_sse._drain_buffer()
		assert_signal_emit_count(_sse, "event_received", 1)
		assert_eq(_sse._buffer.get_string_from_utf8(), "data: partial")

	func test_empty_buffer_no_signal() -> void:
		_sse._buffer = PackedByteArray()
		_sse._drain_buffer()
		assert_signal_not_emitted(_sse, "event_received")
...
class TestFinish:
	...
	func test_finish_clears_buffer() -> void:
		_sse._buffer = "leftover".to_utf8_buffer()
		_sse._finish()
		assert_eq(_sse._buffer.get_string_from_utf8(), "")

	func test_finish_flushes_unterminated_data() -> void:
		_sse._buffer = "data: final chunk".to_utf8_buffer()
		_sse._finish()
...
class TestFail:
	...
	func test_fail_clears_buffer() -> void:
		_sse._buffer = "partial data".to_utf8_buffer()
		_sse._fail("error")
		assert_eq(_sse._buffer.get_string_from_utf8(), "")
...
class TestFinishErrorBody:
	...
	func test_emits_response_error_with_code_and_body() -> void:
		...
		_sse._buffer = '{"error": "bad key"}'.to_utf8_buffer()
...
# A multi-byte UTF-8 character that straddles a chunk boundary must survive
# reassembly. These feed the parser raw bytes split mid-character across two
# reads — the path the String-typed fakes above can't exercise.
class TestMultiByteChunkBoundary:
	extends GutTest
	...
	# Concatenate two PackedByteArrays into a fresh one.
	func _join(a: PackedByteArray, b: PackedByteArray) -> PackedByteArray:
		var out := a.duplicate()
		out.append_array(b)
		return out

	# Feed `first` then `second` as two separate socket reads, draining after
	# each — mirroring _stream_step() accumulating raw bytes across frames.
	func _feed_two_reads(first: PackedByteArray, second: PackedByteArray) -> void:
		_sse._buffer = first
		_sse._drain_buffer()
		_sse._buffer.append_array(second)
		_sse._drain_buffer()

	func test_emoji_split_mid_payload_reassembled() -> void:
		# "data: a😀b\n\n" with the emoji's 4 bytes split 2/2 across reads, so the
		# character is cut in half mid data: payload.
		var emoji := _EMOJI.to_utf8_buffer()
		var first := _join("data: a".to_utf8_buffer(), emoji.slice(0, 2))
		var second := _join(emoji.slice(2), "b\n\n".to_utf8_buffer())
		_feed_two_reads(first, second)
		assert_signal_emitted_with_parameters(
			_sse, "event_received", ["a" + _EMOJI + "b", "message"]
		)

	func test_emoji_split_emits_no_replacement_chars() -> void:
		# Guard against the old per-chunk decode, which left U+FFFD on each half.
		var emoji := _EMOJI.to_utf8_buffer()
		var first := _join("data: ".to_utf8_buffer(), emoji.slice(0, 2))
		var second := _join(emoji.slice(2), "\n\n".to_utf8_buffer())
		_feed_two_reads(first, second)
		var params := get_signal_parameters(_sse, "event_received")
		assert_false(params[0].contains("�"), "no replacement char in payload")

	func test_em_dash_split_at_event_boundary_reassembled() -> void:
		# "data: hi—\n\n" — the em-dash is the last character before the \n\n
		# boundary, and the read splits it 1/2, landing mid-character right at
		# the event boundary.
		var dash := _EM_DASH.to_utf8_buffer()
		var first := _join("data: hi".to_utf8_buffer(), dash.slice(0, 1))
		var second := _join(dash.slice(1), "\n\n".to_utf8_buffer())
		_feed_two_reads(first, second)
		assert_signal_emitted_with_parameters(
			_sse, "event_received", ["hi" + _EM_DASH, "message"]
		)

	func test_partial_char_held_in_buffer_until_completed() -> void:
		# After the first read ends mid-character and before the boundary
		# arrives, nothing is emitted and no character is corrupted.
		var emoji := _EMOJI.to_utf8_buffer()
		_sse._buffer = _join("data: ".to_utf8_buffer(), emoji.slice(0, 2))
		_sse._drain_buffer()
		assert_signal_not_emitted(_sse, "event_received")

	func test_error_body_multibyte_split_reassembled() -> void:
		# The error-body path accumulates raw bytes too; a split character must
		# survive once the full body is decoded.
		_sse._response_code = 400
		var emoji := _EMOJI.to_utf8_buffer()
		_sse._buffer = _join('{"msg":"'.to_utf8_buffer(), emoji.slice(0, 2))
		_sse._buffer.append_array(_join(emoji.slice(2), '"}'.to_utf8_buffer()))
		_sse._finish_error_body()
		assert_signal_emitted_with_parameters(
			_sse, "response_error", [400, '{"msg":"' + _EMOJI + '"}']
		)
